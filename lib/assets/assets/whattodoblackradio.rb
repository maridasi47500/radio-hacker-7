...
system("echo 'klyhgkjhg';mv file5.mp3 filemusic5.mp3;")
@doc = JSON.load(File.read("./sample-data-bblack.json"))
@songs = @doc["songs"]
p @songs.length
n = 0
@all=[]
...
0.upto(temps.length - 2) do |nmusiques|
  ...
  tinsec = (@time_end - temps[nmusiques]["time"]) + 60.to_f + timecut.to_f
    ...
    cut="echo '-#{t1} -#{t2}'>timetable;cutmp3 -i filemusic5.mp3 -f timetable -o #{nom};mid3v2 -a #{temps[nmusiques]["artist"].dump} -t #{temps[nmusiques]["title"].dump} #{nom}0001.mp3;"
    p cut
    system(cut)
    puts("echo '-#{t1} -#{t2}'>timetable;cutmp3 -i filemusic5.mp3 -f timetable -o #{nom};")
end
  end 
  def ecritfichiers
@y=@all.map do |i|
...
\n      #nom2 = temps[nmusiques][:song].parameterize
\n      cut = "rm \#{nom}**.mp3; echo '-\#{t1} -\#{t2}'>timetable;cutmp3 -i filemusic5.mp3 -f timetable -o \#{nom};mv \#{nom}**.mp3 \#{nom}.mp3;mid3v2 -a \#{temps[nmusiques]['artist'].dump} -t \#{temps[nmusiques]['title'].dump} \#{nom}.mp3;"
\n      system(cut)
\n    end
\nend
\nend
...
system(`mkdir -p #{@date}; mv cut.rb #{@date}; mv filemusic5.mp3 #{@date};(cd #{@date} && ruby cut.rb);`);
  end
end
@rad=Radio.new
@rad.ecritfichiers
