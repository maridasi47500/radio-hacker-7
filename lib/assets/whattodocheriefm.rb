...
wow=(`echo 'klyhgkjhg';mv file2.mp3 filemusic2.mp3;`)
@doc = JSON.load(File.read("./sample-data-cheriefm.json"))
@songs = @doc["songs"]
p @songs.length
n = 0
@all=[]
...
0.upto(temps.length - 2) do |nmusiques|
  ...
  tinsec = (@time_end - temps[nmusiques]["time"]) + 60.to_f + timecut.to_f
    ...
    cut="echo '-#{t1} -#{t2}'>timetable;cutmp3 -i filemusic2.mp3 -f timetable -o #{nom};mid3v2 -a #{temps[nmusiques]["artist"].dump} -t #{temps[nmusiques]["title"].dump} #{nom}0001.mp3;"
    p cut
    system(cut)
    #puts("echo '-#{t1} -#{t2}'>timetable;cutmp3 -i filemusic2.mp3 -f timetable -o #{nom};")
end
  end 
  def ecritfichiers
@y=@all.map do |i|
...
\nend
\nend
\nsystem("mv filemusic2.mp3 filemusic.wav;ffmpeg -i filemusic.wav -ar 16000 -ac 1 file.wav;")
\nhack(-460)
\n#mydate and time : lib/assets/history/#{@date}/
\nEOF\nmkdir -p #{@date}; mv cut.rb #{@date}; mv filemusic2.mp3 #{@date};(cd #{@date} && ruby cut.rb);`);
  end
end
@rad=Radio.new
