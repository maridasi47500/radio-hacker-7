...
wow=(`echo 'klyhgkjhg';mv file#{ARGV[0]}.mp3 filemusic#{ARGV[0]}.mp3;`)
@doc = JSON.load(File.read("./sample-data-#{ARGV[0]}.json"))
@songs = @doc["songs"]
p @songs.length
n = 0
@all=[]
@songs.length.times do 
  ...
  n+=1
  p song
  mytime=ARGV[1] == "+" ? time + ARGV[2].to_i.hour + ARGV[3].to_i.minute : time - ARGV[2].to_i.hour - ARGV[3].to_i.minute #decalage horaire radio
  if song.parameterize.length > 0
    @all.push({
        "song"=>song,
        "artist"=>artist,
        "myradio"=>"cheriefm",
...
0.upto(temps.length - 2) do |nmusiques|
  ...
  tinsec = (@time_end - temps[nmusiques]["time"]) + 60.to_f + timecut.to_f
    ...
    cut="echo '-#{t1} -#{t2}'>timetable;cutmp3 -i filemusic#{ARGV[0]}.mp3 -f timetable -o #{nom};mid3v2 -a #{temps[nmusiques]["artist"].dump} -t #{temps[nmusiques]["title"].dump} #{nom}0001.mp3;"
    p cut
    system(cut)
    #puts("echo '-#{t1} -#{t2}'>timetable;cutmp3 -i filemusic#{ARGV[0]}.mp3 -f timetable -o #{nom};")
end
  end 
  def ecritfichiers
@y=@all.map do |i|
...
\n      #nom2 = temps[nmusiques][:song].parameterize
\n      cut = "rm \#{nom}**.mp3; "
\n      cut += "sox file.wav \#{nom}.wav trim \#{i == 0 ? "" : "-"}\#{t2.split(":")[0].to_i*60+t2.split(":")[1].to_i} -\#{t1.split(":")[0].to_i*60+t1.split(":")[1].to_i};"
\n      cut+="mid3v2 -a \#{temps[nmusiques]['artist'].dump} -t \#{temps[nmusiques]['title'].dump} \#{nom}.wav;"
\n      system(cut)
\n    end
\nend
\nend
\nsystem("mv filemusic#{ARGV[0]}.mp3 filemusic.wav;ffmpeg -i filemusic.wav -ar 16000 -ac 1 file.wav;")
\nhack(-460)
